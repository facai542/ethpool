...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`💱 执行货币兑换: ${userAddress || userId} - ${amount} ${fromCurrency} -> ${toCurrency}`)
    ...
    if (userAddress) {
      ...
      if (newUser && !newUserError) {
        ...
        console.log('✅ 从nh_member_new表找到用户:', userData.id)
      } else {
        // 回退到旧表查询
    ...
    if (userData.id && typeof userData.id === 'string' && userData.id.includes('-')) {
      ...
      if (fromCurrency === 'ETH' && toCurrency === 'USDT') {
        ...
        console.log('✅ ETH兑换USDT计算:')
        console.log(`   - 当前可兑换ETH: ${currentEth}`)
        console.log(`   - 兑换金额: ${amount}`)
        console.log(`   - 兑换后剩余ETH: ${newEth}`)
        console.log(`   - 获得USDT: ${exchangedAmount}`)
      }
      ...
      console.log('✅ 更新nh_member_new表余额 - ETH兑换USDT')
      console.log(`   - 总产量ETH: ${currentAeth} -> ${newAeth} (保持不变)`)
      console.log(`   - 可兑换ETH: ${currentEth} -> ${newEth}`)
      console.log(`   - 已兑换USDT: ${currentUsdt} -> ${newUsdt}`)
      console.log(`   - 可提取USDT: ${currentWithdrawableUsdt} -> ${newWithdrawableUsdt}`)
    } else {
      // 数字ID，更新nh_member表
    ...
    if (financeOrderError) {
      ...
      console.log('✅ 兑换交易已记录到finance_orders表')
    }
