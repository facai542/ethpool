...
export class SignatureService {
  ...
  constructor(config: SignatureConfig) {
    ...
    if (!/^[a-fA-F0-9]{64}$/.test(cleanPrivateKey)) {
      throw new Error(`无效的私钥格式: 期望64位十六进制字符串，实际长度: ${cleanPrivateKey.length}`);
    }
    ...
    try {
      ...
      // 使用私钥创建钱包实例（ethers.js会自动添加0x前缀）
      this.signer = provider 
        ? new ethers.Wallet(`0x${cleanPrivateKey}`, provider)
        : new ethers.Wallet(`0x${cleanPrivateKey}`);
    } catch (error) {
      throw new Error(`创建钱包实例失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
...
export const createSignatureService = () => {
  ...
  if (!/^[a-fA-F0-9]{64}$/.test(cleanPrivateKey)) {
    throw new Error('无效的私钥格式，私钥必须是64位十六进制字符串');
  }
  ...
  try {
    ...
    throw new Error(`创建签名服务失败: ${error instanceof Error ? error.message : '未知错误'}`);
  }
}; 