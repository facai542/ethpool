...
interface MetaMaskFoxAvatarProps {
  size?: number
  className?: string
}

export function MetaMaskFoxAvatar({ size = 40, className = '' }: MetaMaskFoxAvatarProps) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const rotateX = Math.max(-maxRotation, Math.min(maxRotation, (mouseY / rect.height) * maxRotation))
      const rotateY = Math.max(-maxRotation, Math.min(maxRotation, (mouseX / rect.width) * maxRotation))
      
      // 平滑动画
      const smoothMove = () => {
        avatar.style.setProperty('--mouse-x', `${rotateX}deg`)
        avatar.style.setProperty('--mouse-y', `${rotateY}deg`)
        
        // 添加轻微的缩放效果
        const distance = Math.sqrt(mouseX * mouseX + mouseY * mouseY)
        const scale = Math.max(1, Math.min(1.2, 1 + (distance / 500)))
        avatar.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) scale(${scale})`
      }
    ...
    const handleMouseEnter = () => {
      setIsFollowing(true)
      avatar.classList.add('mouse-3d')
      document.addEventListener('mousemove', handleMouseMove)
    }

    const handleMouseLeave = () => {
      setIsFollowing(false)
      avatar.classList.remove('mouse-3d')
    ...
    const handleClick = () => {
      avatar.classList.add('mouse-follow')
      setTimeout(() => {
        avatar.classList.remove('mouse-follow')
      }, 600)
    }
  ...
  return (
    <div
      ref={avatarRef}
      className={`metamask-fox-avatar ${isFollowing ? 'mouse-3d' : ''} ${
        isConnected ? 'fox-avatar-glow' : ''
      } ${className}`}
      style={{
        width: size,
        height: size,
...
export default MetaMaskFoxAvatar 