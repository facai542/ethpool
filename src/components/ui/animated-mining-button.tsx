...
interface AnimatedMiningButtonProps {
  onClick?: () => void;
  disabled?: boolean;
  className?: string;
}

const AnimatedMiningButton: React.FC<AnimatedMiningButtonProps> = ({ 
  onClick, 
  disabled = false,
  className = ""
}) => {
...
const StyledWrapper = styled.div`
  .button {
    ...
    --color-background-hover: #16a34a;
    --color-outline: #22c55e8e;
  ...
  .button:hover:not(:disabled) svg polygon:nth-child(1) {
    transform: translateX(0%);
    animation: opacity 1s infinite 0.6s;
  }

  .button:hover:not(:disabled) svg polygon:nth-child(2) {
    transform: translateX(0%);
    animation: opacity 1s infinite 0.4s;
  }

  .button:hover:not(:disabled) svg polygon:nth-child(3) {
...
export default AnimatedMiningButton;
