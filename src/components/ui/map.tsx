...
export function WorldMap({ 
  ...
  const svgMap = useMemo(
    () => map.getSVG({
      radius: 0.22,
      color: theme === "dark" ? "#FFFF7F40" : "#FFFF7F60",
      shape: "circle",
      backgroundColor: theme === "dark" ? "black" : "#0a0a0a",
    }),
    [map, theme]
  );
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-center h-full">
                        <span className="text-xs font-medium px-1.5 py-0.5 rounded-md bg-yellow-400/90 text-black border border-yellow-500 shadow-sm">
                          {dot.start.label}
                        </span>
                      </div>
                    </foreignObject>
      ...
      <AnimatePresence>
        {hoveredLocation && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-4 left-4 bg-yellow-400/90 text-black px-3 py-2 rounded-lg text-sm font-medium backdrop-blur-sm sm:hidden border border-yellow-500"
          >
            {hoveredLocation}
          </motion.div>
        )}
