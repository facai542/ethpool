...
export interface Token {
  symbol: string;
  name: "Ethereum" | "Aave" | string;
  icon?: React.ComponentType<{ className?: string }>;
  iconUrl?: string; // Support for image URLs
}
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="rounded-full h-12 px-4 gap-2 text-lg">
              {selectedToken.iconUrl ? (
                <img src={selectedToken.iconUrl} alt={selectedToken.symbol} className="h-6 w-6 rounded-full" />
              ) : selectedToken.icon ? (
          ...
          <DropdownMenuContent className="w-48">
            {tokens.map((token) => (
              <DropdownMenuItem key={token.symbol} onSelect={() => onTokenSelect(token)}>
                {token.iconUrl ? (
                  <img src={token.iconUrl} alt={token.symbol} className="h-6 w-6 mr-2 rounded-full" />
                ) : token.icon ? (
