...
import StarryAnimation from '@/components/StarryAnimation'
    ...
    <div className="relative w-80 h-80 bg-gradient-to-br from-white via-white/10 to-transparent rounded-3xl p-0.5 shadow-[0_0_80px_-10px_rgba(255,255,255,0.15)]">
      <div className="relative w-full h-full bg-radial-gradient from-gray-700 to-black rounded-3xl flex flex-col overflow-hidden">
        ...
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2">
          <Button className="bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-black font-bold px-6 py-2">
            <DollarSign size={16} className="mr-2" />
            {t.startMining}{symbol}
          </Button>
        </div>
      </div>
    </div>
...
export default function MiningPage() {
  ...
  const fetchInviteStats = async () => {
    ...
    try {
      const response = await fetch(`/api/user/invite-stats?address=${account}`);
      if (response.ok) {
  ...
  const formatBalance = (balance: string | undefined) => {
    return balance ? Number.parseFloat(balance).toFixed(4) : '0.0000'
  }
  ...
  const generateReferralLink = () => {
    if (!inviteStats?.inviteCode) return '';
    
    if (typeof window !== 'undefined') {
      return `${window.location.origin}?ref=${inviteStats.inviteCode}`;
    }
    return `https://your-domain.com?ref=${inviteStats.inviteCode}`;
  };

  const referralLink = generateReferralLink();

  // 默认显示数据（未连接钱包时使用默认值）
  const displayStats = inviteStats || {
    totalReferrals: 0,
    totalVolume: 0,
    totalEarnings: 0,
    commissionRate: 20,
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <header className="flex items-center justify-between p-4 frosted-glass border-b border-border/20 text-[#000000] bg-[#f9f900] font-bold relative z-[11000]">
        <div className="flex items-center gap-4">
          ...
          />
          <LanguageSelector variant="compact" className="slide-in-up" />
        </div>
        
        {isConnected ? (
          <Button 
            onClick={disconnect}
            className="bg-[#f9f900] hover:bg-[#e0e000] text-black border border-black/20 rounded-lg shadow-sm text-xs sm:text-sm px-2 sm:px-4"
          >
            {account ? `${formatAddress(account)} | ${t.disconnectWallet}` : t.disconnectWallet}
          </Button>
        ) : (
          <Button 
            onClick={connect}
            className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold btn-glow"
          >
            {t.connectWallet}
          </Button>
        )}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 bg-black text-white">
        ...
        <div className="space-y-8">
          ...
          >
            <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6 hover:border-yellow-400/50 transition-colors">
              <div className="flex items-center justify-between mb-4">
                <Users className="h-8 w-8 text-yellow-400" />
                <span className="text-2xl font-bold text-yellow-400">{displayStats.totalReferrals}</span>
              </div>
              <h3 className="text-white font-semibold mb-1">总邀请人数</h3>
              <p className="text-gray-400 text-sm">+{displayStats.todayReferrals} 今日新增</p>
            </div>

            <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6 hover:border-green-400/50 transition-colors">
              <div className="flex items-center justify-between mb-4">
                <DollarSign className="h-8 w-8 text-green-400" />
                <span className="text-2xl font-bold text-green-400">{formatCurrency(displayStats.totalVolume)}</span>
              </div>
              <h3 className="text-white font-semibold mb-1">总交易量</h3>
              <p className="text-gray-400 text-sm">USD</p>
            </div>

            <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6 hover:border-purple-400/50 transition-colors">
              <div className="flex items-center justify-between mb-4">
                <Award className="h-8 w-8 text-purple-400" />
                <span className="text-2xl font-bold text-purple-400">{formatCurrency(displayStats.totalEarnings)}</span>
              </div>
              <h3 className="text-white font-semibold mb-1">总收益</h3>
              <p className="text-gray-400 text-sm">佣金</p>
            </div>

            <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6 hover:border-orange-400/50 transition-colors">
              <div className="flex items-center justify-between mb-4">
                <Target className="h-8 w-8 text-orange-400" />
                <span className="text-2xl font-bold text-orange-400">{displayStats.commissionRate}%</span>
              </div>
              <h3 className="text-white font-semibold mb-1">佣金率</h3>
              <p className="text-gray-400 text-sm">永久</p>
            </div>
          </motion.div>

          {/* 邀请工具 - 只在连接钱包且有邀请码时显示 */}
          {isConnected && inviteStats?.inviteCode ? (
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              {/* 邀请码卡片 */}
              <FramedCard minHeight="300px" className="slide-in-up">
                <div className="p-8">
                  <h3 className="text-2xl font-bold text-white mb-6">邀请码</h3>
                  
                  <div className="bg-gray-800/50 rounded-lg p-6 mb-6 border border-gray-600">
                    <div className="text-center">
                      <span className="text-3xl font-mono tracking-wider text-yellow-400">
                  ...
                  <button
                    onClick={() => copyToClipboard(inviteStats.inviteCode, 'code')}
                    className="w-full bg-yellow-400 text-black font-semibold py-3 px-6 rounded-lg hover:bg-yellow-300 transition-colors duration-200 flex items-center justify-center gap-2"
                  >
              ...
              <FramedCard minHeight="300px" className="slide-in-up">
                <div className="p-8">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-2xl font-bold text-white">邀请链接</h3>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      ...
                      className="bg-yellow-400 text-black font-semibold py-3 px-4 rounded-lg hover:bg-yellow-300 transition-colors duration-200 flex items-center justify-center gap-2"
                    >
                      <Copy className="h-4 w-4" />
                      {copiedLink ? '已复制' : '复制'}
                    </button>
                    
                    <button className="bg-blue-600 text-white font-semibold py-3 px-4 rounded-lg hover:bg-blue-500 transition-colors duration-200 flex items-center justify-center gap-2">
            ...
            <motion.div
              ...
              className="mb-8"
            >
              <FramedCard minHeight="200px" className="slide-in-up">
                <div className="p-8 text-center">
                  <Wallet className="h-16 w-16 text-yellow-400 mx-auto mb-6" />
                  <h3 className="text-2xl font-bold text-white mb-4">连接钱包</h3>
                  <p className="text-gray-300 mb-6">连接钱包后查看邀请码和统计数据</p>
                  <Button 
                    onClick={connect}
                    className="bg-yellow-400 text-black font-semibold px-8 py-3 rounded-lg hover:bg-yellow-300 transition-colors duration-200"
                  >
                    连接钱包
                  </Button>
                </div>
          ...
          {/* 邀请用户列表 */}
          {inviteStats?.referredUsers && inviteStats.referredUsers.length > 0 && (
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <FramedCard minHeight="400px" className="slide-in-up">
                <div className="p-8">
                  <h3 className="text-2xl font-bold text-white mb-6">邀请用户</h3>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-gray-600">
                          ...
                          <th className="text-left py-3 px-4 text-gray-400 font-medium">加入日期</th>
                          <th className="text-left py-3 px-4 text-gray-400 font-medium">交易量</th>
                          <th className="text-left py-3 px-4 text-gray-400 font-medium">状态</th>
                        </tr>
                      </thead>
                      <tbody>
                        {inviteStats.referredUsers.map((user) => (
                          <tr key={user.id} className="border-b border-gray-600/50">
                            <td className="py-3 px-4 text-white">{user.username}</td>
                            <td className="py-3 px-4 text-gray-300">
                              {new Date(user.joinDate).toLocaleDateString()}
                            </td>
                            <td className="py-3 px-4 text-white">{formatCurrency(user.volume)}</td>
